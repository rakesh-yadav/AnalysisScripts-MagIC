...
def Ortho_plot(data, ind=0, skip=0, cmap='RdBu_r', vmin=-30000, save_path='/home/rakesh/Desktop', dpi=150, label=r'$(\nabla\times{\bf V})_z$', contour=False, data_cont=[], vmin_cont=1000, l_max_trunc_cont=40):
 ...
 map = Basemap(projection='ortho',lat_0=70,lon_0=90,resolution=None)
 map.drawmeridians(np.arange(0,360,45), dashes=[200,1], latmax=90)
 ...
 if contour==True:
  ...
  levels = np.arange(vmin_cont, data2.max(), 300)
  map.contour(x,y,data2,levels,colors='k',linewidths=[1.5], alpha=0.4)
  levels = np.arange(data2.min(), -vmin_cont, 300)
  map.contour(x,y,data2,levels,colors='k',linewidths=[1.5], linestyles='dashed', alpha=0.4)
  print('Contours plotted')



 # South view
 ax = fig.add_axes([0.45, 0.01, 0.45, 0.97])
 map = Basemap(projection='ortho',lat_0=-70,lon_0=90,resolution=None)
 map.drawmeridians(np.arange(0,360,45), dashes=[200,1], latmax=90)
 map.drawparallels([-60, -30, 30, 60], dashes=[200,1], latmax=90)
 map.drawparallels([0], dashes=[200,1])
